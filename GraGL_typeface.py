...
# This is a slight refactoring of glFreeType.py, written by Sven Olsen in 2003 and ported to
# python by Brian Leair in 2004.
# http://code.google.com/p/rhythmboxcoverflow/source/browse/trunk/artflow/glFreeType.py?r=12
...
def pushScreenCoordinateMatrix():
    ...
    glPushMatrix()
    glLoadIdentity()
    # print viewport[0],viewport[2],viewport[1],viewport[3]
    gluOrtho2D(viewport[0],viewport[2],viewport[1],viewport[3])
    # glOrtho(0, viewport[1], viewport[3], 0, 0, 1)
    glPopAttrib()
    return
...
class TypeFace:
    def __init__ (self, facename, pixel_height):
        ...
        for i in xrange (128):
            # silly me, I'm making this function return the font width. makes things easier.
            self.charWidth[i] = make_dlist (ft, i, self.m_list_base, self.textures);
    ...
    def width(self, string):
        total = 0
        for c in string:
            total += self.charWidth[ord(c)]
        return total

    def textbox(self, string, x, y, w, h):
        lineBreaks = []
        total = 0
        if w < self.charWidth[ord(string[0])]:
            w = self.charWidth[ord(string[0])]
        for i, c in enumerate(string):
            if total + self.charWidth[ord(c)] > w:
                lineBreaks.append(i)
                total = 0
            else:
                total += self.charWidth[ord(c)]
